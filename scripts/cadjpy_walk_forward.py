"""Walk-forward for CADJPY with fwd60 label (ADR-015).

Uses V7 winning features + fwd60 label discovered in isolation test.
"""
...
logger = logging.getLogger('cadjpy_wf')
...
def compute_fwd60_labels(df):
    """Generate fwd60 labels: 60-day forward return classification."""
...
def compute_features_v7(df, macro):
    """V7 features with fwd60 label."""
    labels = compute_fwd60_labels(df)

    labeled = pd.DataFrame({'label': labels})
...
def bootstrap_pvalues(pnl_series, n_iter=500):
    """Bootstrap test: p-value for H0: median PnL <= 0."""
...
def walk_forward():
    ...
    logger.info('Computing V7 features + fwd60 labels...')
    ...
    print(f'  Bootstrap p (median): {bootstrap_pvalues(pd.concat([pd.Series([0]*len(df_w)), df_w["total_return"]])):.4f}')

    min_pf = df_w['profit_factor'].min()
    print()
    print(f'  Gates:')
    print(f'    PF > 1.10:         {"✅" if n_positive_pf >= 5 else "❌"} ({n_positive_pf}/{len(df_w)})')
    print(f'    Bootstrap p < 0.10: {"✅" if (df_w["bootstrap_p"] < 0.10).sum() >= 5 else "❌"} ({(df_w["bootstrap_p"] < 0.10).sum()}/{len(df_w)})')
    print(f'    Min PF > 0.70:     {"✅" if min_pf > 0.70 else "❌"} (min={min_pf:.3f})')

    os.makedirs(os.path.join(BASE, 'data', 'processed'), exist_ok=True)
    out_path = os.path.join(BASE, 'data', 'processed', 'cadjpy_walkforward_results.csv')
