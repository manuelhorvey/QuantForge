"""BTC universe — data loading and feature building for endogenous volatility regime.

BTC uses the existing FeatureContract at FEATURE_REGISTRY["BTC-USD"]
with volatility-adjusted triple barrier labels (tb-vol) instead of standard tb20.
"""
...
logger = logging.getLogger("quantforge.phase_h.btc_universe")
...
def build_btc_features():
    """Build features for BTC using the existing FeatureContract.

    Returns:
        (features_df, available_features) tuple
    """
    ...
    logger.info('  BTC: fetching data...')
    df = fetch_data(years=15)
    logger.info('  BTC: %d rows', len(df))
    ...
    logger.info('  BTC: building features...')
    features_df = build_features(df, macro, ref=None, contract=contract)
    logger.info('  BTC: %d feature rows', len(features_df))

    available = [f for f in feature_names if f in features_df.columns]
    logger.info('  BTC: %d / %d features available', len(available), len(feature_names))

    return features_df, df, available


def create_prediction_frame(df, signal, confidence, name='btc'):
